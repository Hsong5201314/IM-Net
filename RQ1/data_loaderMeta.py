...
class DataProcessor:
    def __init__(self, path, dataset_type='yelp', batch_size=2048):
        ...
        if isinstance(path, dict):
            ...
            self.dual_aux = config.get('dual_aux', False)  # 新增：标志是否启用双辅助任务
            ...
            self.dual_aux = False  # 字符串路径时默认不启用
        ...
        # --- 新增：根据数据集类型动态设置训练负采样数量（Amazon 使用 20，Yelp 保持 10）---
        if self.dataset_type == 'amazon':
            self.train_neg_count = 20
        else:
            self.train_neg_count = 10

        # 3. 创建 PyTorch DataLoaders (多线程 + 锁页内存提速)
        ...
        # 4. 构建 LightGCN 需要的归一化邻接矩阵
        self.norm_adj = self._build_norm_adj()

        # 5. 【资深研究员核心设计】：构建辅助任务的采样池
        self._prepare_auxiliary_sampler()

        # 根据 dual_aux 标志决定是否生成二级辅助边
        if self.dual_aux:
            self._prepare_user_auxiliary()  # 新函数
            self._prepare_item_auxiliary()  # 新函数
        else:
            # 确保即使不生成，这些属性也存在，避免后续采样函数报错
    ...
    def _prepare_user_auxiliary(self):
        """构建用户-用户共现边（基于共同购买的物品数量）- 使用倒排索引高效实现"""
